...
![Architecture Overview](https://cdn.fancia.co.uk/tmp/fancia-infra.png?v=4)
...
#### The Problem with MapStruct in Kotlin

MapStruct was designed for Java. It expects Java-style setters, no-arg constructors, and mutable properties. Kotlin data classes, `val` properties, and constructor-based initialization often prevent MapStruct from generating straightforward mappings.

Common friction points:

- **Immutable targets** — `val` properties cannot be set after construction.
- **No-arg construction** — MapStruct may require a zero-argument constructor or `@JvmOverloads` / `@AllArgsConstructor` workarounds.
- **Annotation processing** — kapt must be configured correctly, and generated code can be harder to debug in Kotlin projects.
- **Spring integration** — when using MapStruct, `@Mapper(componentModel = "spring")` is usually required, adding more generated surface area.

For complex mappings, Kotlin-native alternatives (e.g. Konvert) or hand-written mappers are often simpler and more idiomatic.

##### Kotlin Defaults vs. MapStruct Runtime Behavior

A Kotlin default value only applies when an object is constructed the normal Kotlin way:
...
MapStruct does **not** construct objects that way. It generates Java mapping code that builds instances through its own process. For fields it does not map from the source, it may leave them **unset or explicitly `null` at runtime** — even when the Kotlin declaration is non-nullable or has a default.

That gap between compile-time types and runtime behavior is the core issue:

| What Kotlin promises | What MapStruct may produce |
|----------------------|----------------------------|
| Non-null types       | `null` for unmapped fields |
| Default values       | Ignored during mapping     |
| Constructor init     | Bypassed by generated Java |

Kotlin’s non-null types are a **compile-time** contract. MapStruct operates at **runtime** through generated Java and does not honor Kotlin property initializers for fields it skips.

##### Extension-Based Manual Mappers

The project uses simple, explicit mapper functions as extension methods on domain or request types. This keeps mapping logic readable, type-safe, and fully under control — with no annotation processing and no surprises at runtime.
